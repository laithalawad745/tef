...
async function kickUnauthorizedUser(chatId, userId, username, reason) {
  try {
    ...
    // انتظار 3 ثواني ثم الطرد (لضمان تحميل كل شيء)
    setTimeout(async () => {
      try {
        console.log(`⚡ تنفيذ طرد المستخدم ${userId}...`);
...
📋 **السبب التقني:** ${reason}
        ...
        console.error(`❌ خطأ في تنفيذ طرد المستخدم ${userId}:`, error);
      }
    }, 3000); // 3 ثواني

  } catch (error) {
    console.error(`❌ خطأ في إعداد طرد المستخدم ${userId}:`, error);
  }
}
